...
As per the survey on research paper in this study, adversarial training is divided and discussed into three section for better understanding approaches: 1) Gradient-based, 2) Augmentation-based and, 3) Other. And, the proposed study is more related to augmentation-based adversarial training.
...
Threshold ($\epsilon$) & \multicolumn{2}{c}{ $0.8$ }\\
\hline
\end{tabular}
...
During the experiment, generalization of the model is calculated by using the accuracy of the target model on test samples, here referred to as original accuracy i.e. the percentage of correct predictions over the entire sample set as mentioned in equation \ref{eq:originalacc}.
 \begin{equation}
    \begin{aligned}
       \mbox{Original Accuracy}=\frac{\mbox{number of correct classification}}{\mbox{total number of inputs}}
...
\end{enumerate}
A target model's accuracy under attack is the percentage of correct predictions given crafted test samples by attack recipes, i.e. adversarial test samples as shown in equation \ref{eq:acc_under}. In the experiment, given legitimate test samples to attack recipes for attacking, first attack recipes decide given single input can be a prominent adversarial input if not then that input is skipped. 
% \begin{equation}
%    \begin{aligned}
%        \mbox{Accuracy under attack}=\frac{\mbox{number of correct classification}}{\mbox{number of  adversarial inputs + number of legitimate inputs }}
%        \label{eq:acc_under}
%    \end{aligned}
%\end{equation}
...
% \begin{equation}
%    \begin{aligned}
%        \mbox{Accuracy under attack}=\frac{\mbox{number of correct attempts }}{\mbox{number of  adversarial inputs}
